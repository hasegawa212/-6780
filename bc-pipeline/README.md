...
ターミナルが苦手でもOK。**このMac（MacBookでもMac miniでも）自身で動かす**いちばん簡単な方法：

1. GitHubでこのリポジトリを開き、緑の **「Code」→「Download ZIP」** でダウンロード → ZIPを解凍。
   （または `git clone https://github.com/hasegawa212/-6780.git`）
2. `bc-pipeline` フォルダ内の **`BC自動生成を起動.command`** を **ダブルクリック**。
   - 初回だけ「開発元が未確認」と出たら、ファイルを **右クリック →「開く」→「開く」**（初回のみ）。
3. 黒いウィンドウが開いて自動セットアップ → 数秒で **Safariにログイン画面**が開きます。
   - 初回はAPIキー（無ければ空Enter）とログインユーザーを1回だけ聞かれます。
   - 以降はダブルクリックするだけで起動します。

> これで、その Mac の Safari で `localhost:8800` がそのまま使えます（ネットワーク設定不要）。
> APIキーが無い/無効でも、**物件情報を手入力してBC一式を作成**できます（自動読取のみキー必要）。
...
- [ ] **借地説明書シートへのセル差込**: 実データ入りの借地ワークブック（2通以上）
  ...
  マッピングは当てない**。借地ケースの実例WBを受領次第、`cellmaps` に
  `171-1.借地説明書` 用ビルダーを追加する。
  - 補助ツール `wb_probe.py`（単一WBでも可）で印字ラベルから投入セル候補を
    自動列挙できる（`--preset shakuchi`）。WB受領→候補レビュー→確定の流れに短縮。
    候補は要・実値照合（未照合のまま本番差込に使わない）。
- [ ] **全宅連様式での出力**: 全宅連ブランクWBが未入手のため未対応。受領次第、
  A1 マーカー検出に全宅連バリアントを追加してビルダーを実装する
  （`wb_probe.py` でブランク様式のラベル→候補セルを先行調査できる）。
