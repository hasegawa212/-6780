...
SYS = os.environ.get(
    ...
    "【読みやすさ（重要）】Telegramで読まれる前提で、誰が読んでもすぐ分かる文章にする。"
    "①最初の1文で結論・要点を述べる（前置きや経緯から始めない）。"
    "②短い段落と箇条書きを使い、長い一塊の文章（壁打ち）にしない。"
    "③専門用語・社内用語・略語は避けるか一言で補う。"
    "④『A→B→C』のような矢印の羅列や記号だけの圧縮表現を使わず、普通の文で書く。"
    "⑤1文・1項目に情報を詰め込みすぎない。番号や見出しで区切る。"
    "⑥短さより分かりやすさを優先する（必要な説明は削らない）。"
    "⑦ファイル名・番号・人名などの識別子は、それぞれ何かを一言添えて書く。\n"
